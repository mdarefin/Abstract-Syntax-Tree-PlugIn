...
public class InfoGatherer {
	...
	public int getNumberOfUserMethods() {
		...
		for (int i = 0; i < projects.length; i++) {
			...
			//
			try {// throws exceptions if the project is not open or does not
					// exist
					// ---
					// Check and see if we can access the project and if the
					// project might
					// contain java code
					// ---
		...
		// ---
		// Now that we have all the projects that we can access and that might
		// also
		// contain Java code we need to go through them and count the number of
		// user
		// defined methods.
		// ---
		try {
			for (int i = 0; i < javaprojects.size(); i++) {
				...
				// ---
				// Now we must get all the Packages in the current project. To
				// do this we
				// get the package fragments. A package fragment is a portion of
				// the workspace
				// corresponding to an entire package, or to a portion thereof.
				// The distinction
				// between a package fragment and a package is that a package
				// with some name is
				// the union of all package fragments in the class path which
				// have the same name.
				// Note the casting to a IJavaProject we know this is a safe
				// action because
				// we checked the projects nature above
				// ---
				IPackageFragment[] pfrag = ((IJavaProject) javaprojects.get(i))
						.getPackageFragments();

				// ---
				// now we iterator over the packages contained in the current
				// project
				// ---
				for (int k = 0; k < pfrag.length; k++) {

					// ---
					// If the package does not contain Java source code we don't
					// want to look
					// at it. IPackageFragmentRoot.K_SOURCE indicates that the
					// package does contain
					// source IPackageFragmentRoot.K_BINARY would indicate that
					...
					// source we need to get the compilation units contained in
					// it.
					// An ICompilationUnit represents an entire Java compilation
					// unit (.java source file).
					// ---
					ICompilationUnit[] icu = pfrag[k].getCompilationUnits();

					// ---
					// Now we iterate over all the .java files contained in this
					// package
					// ---
					for (int y = 0; y < icu.length; y++) {

						// ---
						// Each java file (compilationUnit) may define multiple
						// types (ie classes).
						// So we must get each type and iterate through them to
						// find the methods.
						// From the types
						// ---
						...
						for (int tc = 0; tc < allTypes.length; tc++) {
							...
							// ---
							// once we have a type be can get all the methods in
							// the type
							// ---
							IMethod[] im = allTypes[tc].getMethods();

							// ----
							// Here we iterate through the methods, we double
							// check and make sure they
							// are user defined print their name and increase
							// our total
							// ---
	...
	public int getNumberOfMethodCalls(IMethod method) {
		...
		if (method == null) {
			System.out
					.println("The IMethod pasted to getNumberOfMethodCalls is null");
			return -1;
		}

		// ---
		// This is a Java language parser for creating abstract syntax trees
		// (ASTs).
		...
		// ---
		// If setResolveBindings(true), the various names and types appearing in
		// the
		// AST can be resolved to "bindings" by calling the resolveBinding
		// methods.
		// These bindings draw connections between the different parts of a
		// program,
		// and generally afford a more powerful vantage point for clients who
		// wish
		...
		if (icu == null) {
			System.out
					.println("The IMethod past to getNumberOfMethodCalls is not user defined");
			return -1;
		}
		...
		// ---
		// Creates an abstract syntax tree. We pass null for the progress
		// monitor
		// ---
		ASTNode node = parser.createAST(null);
		// ---
		// ASTs support the visitor pattern. Read the notes in the
		// MyVisitor.java file.
		// ---
		MyVisitor visitor = new MyVisitor();
		node.accept(visitor);

		System.out.println("There were " + visitor.getNumberOfNodes()
				+ " node in " + method.getCompilationUnit().getElementName());
